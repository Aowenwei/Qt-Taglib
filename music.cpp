 ...
 * @LastEditTime: 2021-04-10 23:38:23
...
Music::Music(QWidget *parent) : QMainWindow(parent), ui(new Ui::Music) {
  ...
  connect(MyPlay->Play, &QMediaPlayer::durationChanged, this, [=]() {
    ...
    if (MyPlay->state == 0) {
      QString songName = colle->getCollectTabW()->item(curindex, 0)->text();
      lyric->get_song_ID(songName);
      lyric->readLyricFile(songName);
      getCurrMp3Image(colle->Mp3Path.at(curindex));
    }
    if (MyPlay->state == 1) {
      QString songName = MyPlay->getTableW()->item(curindex, 0)->text();
      lyric->get_song_ID(songName);
      getCurrMp3Image(MyPlay->getPath().at(curindex));
    }

    if (MyPlay->state == 2) {
      QString songName = search->getSearchTableW()->item(curindex, 0)->text();
      lyric->Details();
      lyric->get_song_ID(songName);
